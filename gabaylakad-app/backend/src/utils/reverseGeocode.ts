...
export async function findNearestPOI(lat: number, lon: number, radiusMeters: number = 2000) {
  try {
    ...
    const sortedPOIs = (results as LocationIQPOI[]).map((poi: LocationIQPOI) => {
    ...
    }).sort((a: any, b: any) => a.poi_distance_km - b.poi_distance_km);
    ...
    for (const type of priorityTypes) {
      const found = sortedPOIs.find((poi: any) => poi.osm_key === type || poi.type === type);
      if (found) {
        bestPOI = found;
        break;
      }
...
export async function reverseGeocodeNominatim(lat: number, lon: number): Promise<ReverseGeocodeResult> {
  try {
    ...
    let poi_type: string | null = null;
    let poi_name: string | null = null;
    let poi_lat: number | null = null;
    let poi_lon: number | null = null;
    let poi_distance_km: number | null = null;
    let poi_distance_m: number | null = null;
