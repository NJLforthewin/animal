...
export const getAllBatteries = async (req: Request, res: Response) => {
    try {
        const [rows] = await pool.query('SELECT * FROM battery');
        res.json(rows);
    } catch (error) {
...
export const getBatteryById = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const [rows] = await pool.query('SELECT * FROM battery WHERE battery_id = ?', [id]);
...
export const updateBattery = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const battery = req.body;
        const [result] = await pool.query('UPDATE battery SET ? WHERE battery_id = ?', [battery, id]);
...
export const deleteBattery = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const [result] = await pool.query('DELETE FROM battery WHERE battery_id = ?', [id]);
